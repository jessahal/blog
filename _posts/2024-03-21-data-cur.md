...
image: "/assets/img/purple_car.jpeg"
display_image: false
...
Now that we have our URL, we can use the Requests package and command {%- highlight python -%}requests.get(URL, PARAMS){%- endhighlight python -%} to start using Ergast's API to grab our data. We'll then run a quick check to make sure our url worked, and then use {%- highlight python -%} .json()
{%- endhighlight python -%} to convert the results from a JSON file to a Python dictionary that we can access and manipulate easier.
...
After that, you got your data set!
